(function ($) {
    ...
    -------------------------------------------*/
    if ($('.clock').length) {
        function timeElapseClock($el, date) {
            ...
            var html = '<div class="box"><div>' + days + '</div> <span>' + $el.data('text-day') + '</span></div>' +
                '<div class="box"><div>' + hours + '</div> <span>' + $el.data('text-hour') + '</span> </div>' +
                '<div class="box"><div>' + minutes + '</div> <span>' + $el.data('text-minute') + '</span> </div>' +
                '<div class="box"><div>' + seconds + '</div> <span>' + $el.data('text-second') + '</span></div>';
            $el.html(html);
        }

        $('.clock').each(function () {
            var $el = $(this);
            var time = $el.data('date');
            // init countdown plugin for this element
            $el.countdown(time.replace(/-/g, '/'), function (event) {
                if (event.type == 'stoped') {
                    var together = new Date($el.data('date'));
                    together.setHours(0);
                    together.setMinutes(0);
                    together.setSeconds(0);
                    together.setMilliseconds(0);
                    setInterval(function () {
                        timeElapseClock($el, together);
                    }, 1000);
                } else {
                    $el.html(event.strftime(''
                        + '<div class="box"><div>%D</div> <span>' + $el.data('text-day') + '</span> </div>'
                        + '<div class="box"><div>%H</div> <span>' + $el.data('text-hour') + '</span> </div>'
                        + '<div class="box"><div>%M</div> <span>' + $el.data('text-minute') + '</span> </div>'
                        + '<div class="box"><div>%S</div> <span>' + $el.data('text-second') + '</span> </div>'));
                }
            });
        });
    }
